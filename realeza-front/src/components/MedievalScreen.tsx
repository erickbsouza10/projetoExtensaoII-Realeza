...
export function RealezaBrand({ compacta = false }: { compacta?: boolean }) {
  return (
    <div className={`realeza-brand${compacta ? ' brand-compacta' : ''}`}>
      <img className="realeza-brasao" src={logoNassau} alt="Brasao Nassau" />
      <div>
        <h1>REALEZA</h1>
        <span>O reino do conhecimento</span>
      </div>
    </div>
  );
}

export default function MedievalScreen({
  children,
  className,
}: {
  children: ReactNode;
  ...
  return (
    <main className={`realeza-screen ${className}`} onClickCapture={somDoBotao}>
