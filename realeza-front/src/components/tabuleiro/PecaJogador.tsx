...
export default function PecaJogador({ jogador, x, y, deslocamento, movendo }: Props) {
  return (
    <div
      ...
      style={{ left: `calc(${x}% + ${x / 100 - 0.5} * var(--gap-circuito))`, top: `calc(${y}% + ${y / 100 - 0.5} * var(--gap-circuito))`, "--cor-peca": jogador.cor, "--deslocamento": `${deslocamento / 34 * 100}%` } as CSSProperties}
    ...
    >
      {jogador.imagem ? <img src={jogador.imagem} alt="" /> : (
